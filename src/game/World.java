...
import game.behavior.AIBehaviour;
import game.behavior.AnimationBehaviour;
import game.behavior.AnimationStateBehaviour;
import game.behavior.InputBehaviourOld;
import game.behavior.MotionBehaviour;
...
/**
 * The World class holds all game objects and establishes communication among
 * them. Handles creation and destruction of entities.
 */
public class World implements Drawable {
	static ArrayList<GameObject> gameObjects; // All game objects in the game
												// are stored here.
	static Clock timer = new Clock(); // doesnt belong here
	static Clock timerShoot = new Clock(); // doesnt belong here
	static int playerIndex = -1; // index of the player object in the array of
									// game objects.
	 ...
	 */
	public void update() {
		if (GameWindow.inputHandler.isMouseClicked == true && timerShoot.getElapsedTime().asSeconds() > 0.1f) {
			timerShoot.restart();
			createProjectile(Game.resources.textures.get(3), gameObjects.get(0).sprite.getPosition(),
					gameObjects.get(0).motion.direction);
		}
	...
	/**
	 * Resolves all collisions that happen. Still not finished. Works only to
	 * kill enemies.
	 */
	 ...
	 *
	 * @param A
	 *            sprite of first entity
	 * @param B
	 *            sprite of second entity
	...
	public boolean isCollision(Sprite A, Sprite B) {
		...
		Vector2f bottomRightA = new Vector2f(topLeftA.x + A.getGlobalBounds().width,
				topLeftA.y + A.getGlobalBounds().height);
		Vector2f topLeftB = new Vector2f(B.getGlobalBounds().left, B.getGlobalBounds().top);
		Vector2f bottomRightB = new Vector2f(topLeftB.x + B.getGlobalBounds().width,
				topLeftB.y + B.getGlobalBounds().height);
	...
	public static void createPlayer() {
		...
		// object.addBehaviour( new InputBehaviourOld( Game.inputHandler,
		// object.motion, object.abilities ) );
		...
		object.addBehaviour(new AnimationBehaviour(object.sprite, 14, 21, 7, 28),
				GameObject.Ability.MOVE_LEFT.ordinal());
		object.addBehaviour(new AnimationBehaviour(object.sprite, 21, 28, 7, 28),
				GameObject.Ability.MOVE_RIGHT.ordinal());
	...
	public static void createPlayerBeta() {
		...
		object.addBehaviour(new MotionBehaviour(object.sprite));
		object.addBehaviour(new InputBehaviourOld(GameWindow.inputHandler, object.motion, object.sprite, object.abilities));
	...
	public static void createDummy() {
		...
		object.addBehaviour(new AnimationBehaviour(object.sprite, 21, 28, 7, 28),
				GameObject.Ability.MOVE_RIGHT.ordinal());
		object.addBehaviour(new AnimationStateBehaviour(object.anims, object.abilities));
		object.sprite.setPosition(new Vector2f(GameWindow.SCREEN_WIDTH / 2, GameWindow.SCREEN_HEIGHT / 2));
	}

	/**
	 * Use this to create tiles, walls, chests, whatever. If you don't know how
	 * to use it, see the example in the constructor.
	 */
	 ...
	 *
	 * @param tex
	 *            the texture that is going to be used for the projectile.
	 * @param pos
	...
	public static void destroyOutOfBoundsObjects() {
		...
		for (int i = gameObjects.size() - 1; i > 0; i--) // Counts backwards for
															// performance
															// reasons.
		{
	...
	public static void destroyDeadObjects() {
		for (int i = gameObjects.size() - 1; i > 0; i--) {
			if (gameObjects.get(i).status == GameObject.Status.DEAD.ordinal()) {
				gameObjects.remove(gameObjects.get(i));
			}
		}
	}
	...
	public static void createEnemyRandom() {
		...
		object.sprite
				.setPosition(new Vector2f(((float) Math.sin(Math.toRadians(angle))) * 500.0f + GameWindow.SCREEN_WIDTH / 2,
						((float) Math.cos(Math.toRadians(angle))) * 500.0f + GameWindow.SCREEN_HEIGHT / 2));
