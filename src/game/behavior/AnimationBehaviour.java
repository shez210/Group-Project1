...
public class AnimationBehaviour {
	...
	private final int SEQUENCE_SPEED_NORMAL = 80; // Milliseconds spent on 1
													// sprite.
	...
	private int sequenceStart; // The index of the first sprite that is going to
								// be used from the sprite sheet.
	private int sequenceEnd; // The index of last sprite from sprite sheet.
	private int millisPerSequence = SEQUENCE_SPEED_NORMAL;
	private int numSequencesRow; // Number of sprites per row in the sprite
									// sheet.
	private int numSequencesColumn; // Number of sprites per column in the
									// sprite sheet.

	// Used for idle animations.
	private boolean showOnlyOneFrame; // Supports the option to show only the
										// first frame of a certain animation.

	/**
	 * Automatically animates the sprite based on the spritesheet provided.
	 * 
	 * @param spr
	 *            the sprite that is going to be animated.
	 * @param numSequencesRow
	 *            the number of sprites per row in the sprite texture. for
	 *            example there are 15 sprites per row in this texture here:
	 *            http://www.gamefromscratch.com/image.axd?picture=a.png
	 * @param totalSequences
	 *            the total number of sprites in the spritesheet/texture. in the
	 *            link above, there are total of 32 sprites.
	 */
	public AnimationBehaviour(Sprite spr, int sequenceStart, int sequenceEnd, int numSequencesRow, int totalSequences) {
		...
		this.numSequencesColumn = (totalSequences % numSequencesRow == 0) ? totalSequences / numSequencesRow
				: totalSequences / numSequencesRow + 1;
