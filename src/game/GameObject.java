...
import game.behavior.InputBehaviour;
...
public class GameObject {
	...
	// GameObject is created by instantiating all the needed
	...
	void update() {
		if (input != null) {
			input.update();
		} // If the GameObject is controlled by input, handle the input.
		if (motion != null) {
			motion.update();
		} // If the GameObject can move, then move it.
		if (animState != null) {
			animState.update();
